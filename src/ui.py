...
class ClinicalDataUI:
    ...
    def retrieve_patient(self):
        ...
        data = self.patients.read_all()

        patient_rows = [row for row in data if row['Patient_ID'].strip() == pid.strip()]
    ...
    def remove_patient(self):
        ...
        if not pid:
            return
        success = self.patients.remove_patient(pid)
        if success:
            ...
            messagebox.showerror("Error", f"Patient {pid} not found.")
